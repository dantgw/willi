...
contract MultisigPlugin is BasePlugin, IWillPlugin, IERC1271 {
    using AssociatedLinkedListSetLib for AssociatedLinkedListSet;
    using ECDSA for bytes32;
    using SafeCast for uint256;

    string internal constant _NAME = "Multisig Will Plugin";
    ...
    // Add mapping to track original owners
    mapping(address account => address originalOwner) internal _originalOwners;
    ...
    error NotAnOwner();
    error ProofOfLifePeriodNotElapsed(); 
    ...
    function onUninstall(bytes calldata) external override {
        ...
        _ownerMetadata[msg.sender] = OwnershipMetadata(0, 0, 0 , 0);
        emit OwnerUpdated(msg.sender, new address[](0), ownersToRemove, 0);
    }

    /// @notice Validate UserOps (bundler validation)
        ...
        returns (uint256)
    {
        if (functionId == uint8(FunctionId.USER_OP_VALIDATION_OWNER)) {
            ...
            bytes memory signature = userOp.signature[96:];
            address signer = _recoverSigner(userOpHash, signature);
            
            // If signer is not the original owner, check the time constraint
            if (signer != _originalOwners[msg.sender]) {
                OwnershipMetadata storage metadata = _ownerMetadata[msg.sender];
                uint256 timeSinceLastActive = block.timestamp - metadata.lastActiveTime;
                if (timeSinceLastActive < metadata.proofOfLifePeriod) {
                    return SIG_VALIDATION_FAILED;
                }
            }

            // Rest of existing signature validation code...
    ...
    /// @notice Validate regular transactions (runtime validation)
    function runtimeValidationFunction(uint8 functionId, address sender, uint256 value, bytes calldata data)
        external
        virtual
        override
    {
        // Check if sender is either the original owner or a registered owner
        bool isOriginalOwner = (sender == _originalOwners[msg.sender]);
        bool isRegisteredOwner = _owners.contains(msg.sender, CastLib.toSetValue(sender));
        
        if (!isOriginalOwner && !isRegisteredOwner) {
            revert NotAnOwner();
        }

        // If sender is not the original owner, apply time constraints
        if (!isOriginalOwner) {
            OwnershipMetadata storage metadata = _ownerMetadata[msg.sender];
            uint256 timeSinceLastActive = block.timestamp - metadata.lastActiveTime;
            
            // Check if enough time has passed (proof of life period)
            if (timeSinceLastActive < metadata.proofOfLifePeriod) {
                revert ProofOfLifePeriodNotElapsed();
            }
        }
    }
    ...
    function _onInstall(bytes calldata data) internal override isNotInitialized(msg.sender) {
        ...
        _ownerMetadata[msg.sender] = OwnershipMetadata(uint128(initialOwners.length), threshold, 0, 31560000);
        
        // Store the first owner as the original owner
        _originalOwners[msg.sender] = initialOwners[0];
    ...
    /// @notice Update lastActiveTime before executing any transaction
    ...
    {
        // Update lastActiveTime whenever any transaction is executed
